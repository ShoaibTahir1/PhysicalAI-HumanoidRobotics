...
# Fixed script to update AITutor component in all documentation files
# Only adds the component if it's not already present
# Run from the root directory of the project

echo "Updating documentation files with enhanced AITutor component..."
...
for file in docs/*.md; do
    if [ -f "$file" ]; then
        ...
        # Check if AITutor component is already present in the file
        if grep -q "AITutor" "$file"; then
            echo "  → AITutor component already present, skipping..."
            continue
        fi
...
echo "All documentation files updated with enhanced AITutor component!"